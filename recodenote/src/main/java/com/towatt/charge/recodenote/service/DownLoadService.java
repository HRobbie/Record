...
public class DownLoadService extends Service {
    ...
    private String path=BASE_PATH+"recordNote"+"/recordNote.apk";
...
//        downloadFile(download_url);
        ...
        builder.setSmallIcon(R.mipmap.ic_launcher).setContentTitle("语音记事本正在下载0%").setWhen(System.currentTimeMillis())
                .setPriority(Notification.PRIORITY_MAX).setTicker("语音记事本下载").setOngoing(false).setAutoCancel(true)
...
//        requestParams.setAutoRename(true);//断点下载
        ...
        x.http().get(requestParams, new Callback.ProgressCallback<File>() {
            ...
            public void onLoading(long total, long current, boolean isDownloading) {
                builder.setContentTitle("语音记事本正在下载"+(int)((double)current/total*100)+"%").setProgress((int)total,(int)current,false);
               send();
            ...
            public void onSuccess(File result) {
                builder.setContentTitle("语音记事本下载完成100%").setProgress(100, 100, false);
                send();
                openFile(result);
            }
            ...
            public void onError(Throwable ex, boolean isOnCallback) {
                CommentUtils.showToast(DownLoadService.this,"下载失败，请检查网络和SD卡");
            }
